...
The script copies the framework files in; existing `AGENTS.md`, `method.config.yaml`, and `docs/adr/` are preserved. Re-run the same command to upgrade — it only overwrites framework files. Pin a version with `sh -s -- --ref v2.2.2`. After a manual install you'll customise `AGENTS.md` and `method.config.yaml` and connect your MCPs yourself (credentials live in `~/.claude/mcp.json`, never in the repo).
