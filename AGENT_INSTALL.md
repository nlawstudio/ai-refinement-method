...
|---|---|---|
| `docs/domain-glossary.md` | Ubiquitous language — domain terms, one agreed definition each | Every project. Created and maintained by the Explorer via `/storm`; no setup needed now. |
| `docs/schema.sql` | Database DDL | Any project with a database. Highest leverage of the generated refs. |
...
The **domain glossary** is created automatically the first time the human runs `/storm` — you don't set it up during install. The rest are project-generated; walk the human through the ones that fit.

### 5a. Schema export

If the human said they use a database in Step 1:

> "The method becomes dramatically more useful when it has the schema at session start. Every Designer, Architect, and Test Author session sees the actual data shape — no guessing, no greps."
>
...
> "That's fine — the git tree.yaml becomes the operational state. The team picks up 'ready' leaves from `plans/{epic}/tree.yaml` directly instead of from a tracker. `/plan` operates in dry-run mode (produces all git artifacts; no remote push)."
...
> "Logged as a follow-up. Without a tracker MCP connected, `/plan` operates in dry-run mode — it produces all the git artifacts (tree, domain map, ADRs, threat model, tests, manifest) but doesn't push to the tracker. Everything else works without it."
...
- "Map the domain for X" → routes to an event-storming session (the Explorer)
...
- **Doesn't write production code.** The Method produces ready specs; implementation happens in their own coding tool. The code in their project is theirs.
...
- **Stops at the spec.** Its output is DoR-ready stories with failing tests, decisions, threat models, and a domain map — a developer (or their coding agent) builds from there.
